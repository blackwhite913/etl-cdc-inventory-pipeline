...
import type { CwStockItem } from "@/services/cw-stock.service";
import { runCwStockEtlAction } from "./actions";
...
export default function CwLogisticsPage() {
  const [items, setItems] = useState<CwStockItem[]>([]);
  ...
  const loadPage = useCallback(async (page: number, options?: { silent?: boolean }) => {
    ...
    try {
      ...
      const response = await fetch(`/api/cw-stock?${params.toString()}`, {
        method: "GET",
      });
      const body = (await response.json()) as CwStockResponse;
      ...
      const message =
        err instanceof Error ? err.message : "Unexpected error while loading CW Logistics stock";
      setError(message);
      if (!silent) {
        setItems([]);
        setTotal(0);
  ...
  const pollCwStatus = useCallback(async () => {
    try {
      const res = await fetch("/api/cw-stock-status");
      if (!res.ok) return;
      const data = (await res.json()) as CwStockStatusResponse;
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => void pollCwStatus(), ETL_STATUS_POLL_MS);
    return () => {
  ...
  useEffect(() => {
    if (!etlBanner) return;
    const timeout = setTimeout(() => setEtlBanner(null), 8_000);
  ...
  const handleRunEtl = () => {
    ...
    startTransition(async () => {
      try {
        const summary = await runCwStockEtlAction();
        if (summary.status === "FAILED") {
          setEtlBanner({
            type: "error",
            message: `CW Logistics ETL failed: ${summary.errorMessage ?? "unknown error"}`,
          });
        } else {
          const partialLabel =
            summary.partialFailures > 0
              ? ` with ${summary.partialFailures} partial page failures`
              : "";
          setEtlBanner({
            type: "success",
            message: `CW Logistics sync complete — inserted ${summary.inserted}, updated ${summary.updated} (${summary.totalTimeMs}ms)${partialLabel}.`,
          });
        }
      } catch (err) {
        setEtlBanner({
          type: "error",
          message: err instanceof Error ? err.message : "CW Logistics ETL action failed",
        });
      } finally {
        void loadPage(currentPage, { silent: true });
        void pollCwStatus();
  ...
  const openSchema = useCallback(() => {
    ...
    setSchemaError(null);
    void fetch("/api/db/schema?tables=cw")
      .then(async (res) => {
    ...
    <main className="min-h-screen bg-[#0B0F14] px-4 py-6 text-slate-100 md:px-8">
      <div className="mx-auto flex w-full max-w-[1800px] flex-col gap-6">
        <header className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-[#111827]/60 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-xl font-semibold tracking-tight">CW Logistics</h1>
            <p className="mt-0.5 text-xs text-slate-400">
              {syncLabel} {rowCount > 0 ? `· ${rowCount.toLocaleString()} rows` : ""}
            </p>
          </div>
                        ...
                        <tr>
                          <td colSpan={8} className="px-3 py-8 text-center text-slate-400">
                            {debouncedSearch.length > 0
                              ...
                              : "No CW Logistics stock yet. Click \"Fetch Data\" to run the ETL and load this warehouse."}
                          </td>
                        </tr>
                      ) : (
